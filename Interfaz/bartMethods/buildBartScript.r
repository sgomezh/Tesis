...
sink("bartMethods/summary.txt", append = FALSE)

option_list = list(
    make_option(c("-f", "--file", type="character", default="",
            help="Path of the dataset", metavar="character")),
    make_option(c("-cv", "--crossValidate", type="logical", default=FALSE,
            help="Cross validate the model", metavar="logical")), 
    ...
    make_option(c("-burn", "--burnIn", type="integer", default=100,
            help="Number of burn in iterations", metavar="integer")),
    make_option(c("-pburn", "--postBurnIn", type="integer", default=100,
    ...
    make_option(c("-k", type="numeric", default=2,
...
placed", metavar="numeric")),
    make_option(c("-nu", "--nu", type="numeric", default=3,
            help="Degrees of freedom for x^2 prior", metavar="numeric")),
    make_option(c("-g", "--grow", type="integer", default=1,
            help="Grow percentage for MH", metavar="numeric")),
    make_option(c("-p", "--prune", type="integer", default=1,
            help="Prune percentage for MH", metavar="numeric")),
    make_option(c("-c", "--change", type="integer", default=1,
...
bart = build_bart_machine(X = train[, -which(names(train) == opt$response)],
                            ...
                            q = opt$quantiles,
                            nu = opt$nu,
                            mh_prob_steps = c(opt$grow, opt$prune, opt$change))
